...
import { Search, Menu, X, Upload, MessageCircle, User, LogOut } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <header className="h-20 sticky top-0 z-50 glass-effect border-b border-gray-100 shrink-0">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={logout} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors" title="התנתק">
                <LogOut size={20} />
              </button>
            </div>
          ) : (
            <button 
              ...
              className="bg-brand-teal text-white px-5 py-2.5 rounded-full text-sm font-medium shadow-md shadow-brand-teal/20 hover:opacity-90 active:scale-95 transition-all flex items-center gap-2"
            >
              <User size={18} />
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-y-0 right-0 w-64 bg-white z-[51] lg:hidden shadow-2xl p-6"
            >
